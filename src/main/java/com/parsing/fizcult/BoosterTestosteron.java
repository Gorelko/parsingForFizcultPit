...
public class BoosterTestosteron {

    public BoosterTestosteron() throws IOException, InterruptedException {
        ...
        String nameProductGroup = "Повышение тестостерона";



        String link1 = "https://fizcult.by/catalog/povyshenie-testosterona/?PAGEN_1=1&MUL_MODE=";
        ...
        while (true) {
            try {
                ...
                String link = "https://fizcult.by/catalog/povyshenie-testosterona/?PAGEN_1=1&MUL_MODE=";
        ...
        for (int x = 1; x <= countPage; x++) {
            ...
            while (true) {
                try {
                    doc = Jsoup.connect("https://fizcult.by/catalog/povyshenie-testosterona/?PAGEN_1="+x+"&MUL_MODE=").get();
                    Thread.sleep(1000);
                    break;
                }catch (Exception e){
                    System.out.println("Обрыв коннекта, подключаемся! Возможная причина - отсутствие интернета или сайт не работает!");
