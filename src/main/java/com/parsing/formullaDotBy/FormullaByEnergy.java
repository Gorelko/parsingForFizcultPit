...
public class FormullaByEnergy {

    public FormullaByEnergy() throws IOException, InterruptedException {
        ...
        String nameProductGroup = "Энергия";
        ...
        while (true) {
            try {
                ...
                String link = "https://formulla.by/nutrition/energy/?limit=120";
        ...
        for (int x = 1; x <= countPage; x++) {
            ...
            while (true) {
                try {
                    doc = Jsoup.connect("https://formulla.by/nutrition/energy/?limit=120&page="+ x).get();
                    Thread.sleep(1000);
                    break;
                }catch (Exception e){
                    System.out.println("Обрыв коннекта, подключаемся! Возможная причина - отсутствие интернета или сайт не работает!");
