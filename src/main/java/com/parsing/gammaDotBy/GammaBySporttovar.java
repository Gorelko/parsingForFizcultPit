...
public class GammaBySporttovar {

    public GammaBySporttovar() throws IOException, InterruptedException {
        ...
        String nameProductGroup = "Спортивные товары";


        String linkProtein = "https://gamma.by/sport-tovar/?limit=100&page=1";
        Thread.sleep(1000);
        ...
        int maxCountPage = 1;
        try {
            maxCountPage = Integer.parseInt(String.valueOf(countPage).split("\"")[String.valueOf(countPage).split("\"").length-2].split("=")[2]); //максимальное количество страниц
        } catch (Exception e){
        ...
        for (int v = 1; v <= maxCountPage; v++) {
            ...
            link = "https://gamma.by/sport-tovar/?limit=100&page=" + v;
            ...
            for (int i = 0; i < product.size(); i++) {
                ...
                String brandStr = "";
                try {
                    brandStr = String.valueOf(productBrandElement).split("<img src=\"https://gamma.by/image/cache/catalog/image/cache/catalog/")[1].split("/")[0]; //бренд

                }catch (Exception e){
                    brandStr = String.valueOf(productBrandElement).split("src=\"https://gamma.by/image/cache/catalog/")[1].split("/")[0]; //бренд
                }
...
/*                System.out.println(linkProduct);
                ...
                if (weightElementsStr1.length() > 0) {
                    productList.add(productDescriptionStr.replaceAll(",",".") + " (" + weightElementsStr1.replaceAll(",",".") + ")");
                } else {
                ...
                if (weightElementsStr2.length() > 0) {
                    ...
                    productList.add(nameProductGroup);
                    productList.add(brandStr);
                    productList.add(productDescriptionStr.replaceAll(",",".") + " (" + weightElementsStr2.replaceAll(",",".") + ")");
                    productList.add("Вес в описании");
                    productList.add(stock1);
            ...
            for (int i = 0; i < product.size(); i++) {
                ...
                Element productBrandElement = product.get(i).getElementsByAttributeValue("class", "img-responsive").get(0);
                //System.out.println(productBrandElement);
                String brandStr = "";
                try {
                    brandStr = String.valueOf(productBrandElement).split("src=\"https://gamma.by/image/cache/catalog/image/cache/catalog/")[1].split("/")[0]; //бренд
                }catch (Exception e){
                    brandStr = String.valueOf(productBrandElement).split("src=\"https://gamma.by/image/cache/catalog/")[1].split("/")[0]; //бренд
                }
                String productDescriptionStr = String.valueOf(productBrandElement).split("alt=\"")[1].split(" </a> ")[0].replaceAll("\">", ""); //описание
                String image = String.valueOf(productBrandElement).split("\"")[1]; //фото
                //   Element productWeightElement1 = product.get(i).getElementsByAttributeValue("class", "img-hpm-col-model").get(0);
