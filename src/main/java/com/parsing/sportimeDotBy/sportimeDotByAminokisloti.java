...
public class sportimeDotByAminokisloti {

    public sportimeDotByAminokisloti() throws IOException, InterruptedException {
        ...
        while (true) {
            try {
                ...
                String link = "http://sportime.by/aminokisloti";
        ...
        for (int x = 0; x <= countPage; x++) {
            ...
            while (true) {
                try {
                    if (x==0){
                        doc = Jsoup.connect("http://sportime.by/aminokisloti").get();
                    }else {
                        doc = Jsoup.connect("http://sportime.by/aminokisloti?page="+ x).get();
                    }
                    Thread.sleep(1000);
                    break;
                }catch (Exception e){
