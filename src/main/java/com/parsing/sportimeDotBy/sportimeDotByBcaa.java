...
public class sportimeDotByBcaa {

    public sportimeDotByBcaa() throws IOException, InterruptedException {
        ...
        while (true) {
            try {
                ...
                String link = "http://sportime.by/BCAA";
        ...
        for (int x = 0; x <= countPage; x++) {
            ...
            while (true) {
                try {
                    if (x==0){
                        doc = Jsoup.connect("http://sportime.by/BCAA").get();
                    }else {
                        doc = Jsoup.connect("http://sportime.by/BCAA?page="+ x).get();
                    }
                    Thread.sleep(1000);
                    break;
                }catch (Exception e){
