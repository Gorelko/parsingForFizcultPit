...
public class sportimeDotByKreatin {

    public sportimeDotByKreatin() throws IOException, InterruptedException {
        ...
        while (true) {
            try {
                ...
                String link = "http://sportime.by/kreatin";
        ...
        for (int x = 0; x <= countPage; x++) {
            ...
            while (true) {
                try {
                    if (x==0){
                        doc = Jsoup.connect("http://sportime.by/kreatin").get();
                    }else {
                        doc = Jsoup.connect("http://sportime.by/kreatin?page="+ x).get();
                    }
                    Thread.sleep(1000);
                    break;
                }catch (Exception e){
