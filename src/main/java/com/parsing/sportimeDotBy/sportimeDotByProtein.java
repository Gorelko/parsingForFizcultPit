...
public class sportimeDotByProtein {

    public sportimeDotByProtein() throws IOException, InterruptedException {
        ...
        while (true) {
            try {
                ...
                String link = "http://sportime.by/protein";
        ...
        for (int x = 0; x <= countPage; x++) {
            ...
            while (true) {
                try {
                    if (x==0){
                        doc = Jsoup.connect("http://sportime.by/protein").get();
                    }else {
                        doc = Jsoup.connect("http://sportime.by/protein?page="+ x).get();
                    }
                    Thread.sleep(1000);
                    break;
                }catch (Exception e){
